...
module.exports = class GroovyConsoleApplication {
    ...
    _runAllScriptsInCurrentDirectory(options) {
        ...
        fs.readdir(process.cwd(), (err, files) => {
            files = files.filter(file => file.endsWith('.groovy'));
            self._printOutput(files.length === 0, warning("Sorry, but there is no any groovy script to run in the current location"));
